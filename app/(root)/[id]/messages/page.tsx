...
const [userofdb , setuserdb] =useState<any>([])
const [allmessages , setallmessages] =useState([])
const [usertoChat , setuserToChat] = useState<any>([])
const [userTomessage, setuserToMessage] = useState<any>({})
...
useEffect(()=>{
  async function start() {
    if(user?.emailAddresses[0].emailAddress != undefined) {
      const userdb = await getUsers("yaya89yuyu@gmail.com")
...
useEffect(() => {
  ...
  useEffect(()=>{
    const getUser =async() =>{
      const user = await getUsersToMessage(id)
      if(user) {
        setuserToMessage(user as any)
      }
  ...
  useEffect(()=>{
    const getUser =async() =>{
      if(userofdb.$id == undefined) return
      const message = await getAllMessage(userofdb.$id as string)
      console.log(message)
      if(message) {
...
<img 
src={userTomessage && userTomessage.image}
alt=""
width={30}
height={30}
className=" rounded-full"
/>
<p>{userTomessage.name}</p>
</div>
    <Card >
...
<div className=" ml-2 flex w-[300px] flex-col">
{
  ...
  userofdb?.message?.length >0 && userofdb.message.map((msg :any) =>(
...
{
  userTomessage &&
  userTomessage?.message?.length >0 && userTomessage.message.map((msg :any) =>(
  ...
  <img 
  src={userTomessage?.image && userTomessage.image as string}
  alt=""
  width={30}
  height={30}
  className=" rounded-full"
  ...
  <p>{msg.message && msg.message}</p>
    ...
    <Button  onClick={async()=> await CreateMessages({userId :userofdb.$id as string , message :message})} className=" gap-3">
    <Image 
    src="/send.png"
    alt=""
    width={30}
